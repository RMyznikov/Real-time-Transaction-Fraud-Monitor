...
| `web-client` | 1 | Angular monitoring console на `localhost:4200` |
...
Angular web client доступний на `http://localhost:4200`. Він містить:

- форму запуску генератора з Material success dialog;
- MTX/Matero data grid транзакцій із server-side пошуком і пагінацією;
- MTX/Matero data grid fraud alerts;
- Angular HTTP service з повним CRUD-контрактом для transactions і fraud alerts.

Підняти весь стек і відкрити UI у Chrome однією командою на Windows:
...
| `WEB_CLIENT_IMAGE` | Published Angular/Nginx image для registry Compose |
| `WEB_CLIENT_EXTERNAL_PORT` | Зовнішній порт web-client, типово `4200` |
...
WEB_CLIENT_IMAGE=ghcr.io/rmyznikov/real-time-transaction-fraud-monitor-web:v1
